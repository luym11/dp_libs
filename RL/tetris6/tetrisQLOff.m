function DATAout = tetrisQLOff( DATA )
    DATAout = DATA;
    ...
    %     u = randiP(muRand);
        ...
        if size(newBoard,1) > DATA.RowCap
            Q(x,u + moves_array(new_piece)) = (1-step)*Q(x,u + moves_array(new_piece)) + step*score; 
            % Increment js (update step)
            js(x,u) = js(x,u)+1;
            board = newBoard; 
            continue;
        end
        ...
        for p = 1:n_Pieces
            ...
            Q(x,u + moves_array(new_piece)) = (1-step)*Q(x,u + moves_array(new_piece)) + step*(score + alpha*max(Q(next_state,:)));
%           Q(x,u) = Q(x,u) + 1/n_Pieces * alpha * max(Q(next_state, :)); 
        end
        % Increment js (update step)
        js(x,u) = js(x,u)+1;
        board = newBoard; 
